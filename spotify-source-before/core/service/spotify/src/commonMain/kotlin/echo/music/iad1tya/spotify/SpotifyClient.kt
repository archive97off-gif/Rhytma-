...
class SpotifyClient {
    ...
    suspend fun getSpotifyPlaylist(
        ...
        val response = spotifyClient.get("https://api.spotify.com/v1/playlists/$playlistId") {
            userAgent(USER_AGENT)
            header(HttpHeaders.Authorization, "Bearer $authToken")
            parameter("offset", offset)
            parameter("limit", limit)
            parameter("fields", "name,images,tracks(total,next,items(track(name,artists(name),duration_ms)))")
        }
        if (response.status.value !in 200..299) {
            error("Spotify returned HTTP ${response.status.value}")
        }
